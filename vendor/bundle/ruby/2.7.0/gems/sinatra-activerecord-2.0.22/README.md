...
# or set :database_file, "path/to/database-mysql.yml"
```
...
```sh
$ bundle exec rake -T
rake db:create            # Create the database from DATABASE_URL or config/database-mysql.yml for the current Rails.env (use db:create:all to create all dbs in the config)
